...
void tambah(struct Node **root, int dataku){
      if((*root) == NULL){
            struct Node *baru;
            // baru == new Node;  // 'new' kagak ada di C
            baru = (struct Node*)malloc(sizeof(struct Node)); // deklarasi 'new'
            baru->data = dataku;
            baru->kiri = NULL;
            baru->kanan = NULL;
            (*root) = baru;
...
void hapus(struct Node **root, int data) {
	if(data == (*root)->data) { //apakah data ada di root?
		if((*root)->kanan == NULL) { //if anak kanan null maka eksekusi bawah
